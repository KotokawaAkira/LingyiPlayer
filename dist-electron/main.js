...
async function loadLyric(path2) {
  return new Promise((resolve2) => {
    fs.readFile(path2, (err, data) => {
      if (err) resolve2(void 0);
      else resolve2(data.toString());
    });
  });
}
require$$3.app.commandLine.appendSwitch("js-flags", "--expose-gc");
remoteMain.initialize();
let window$1;
const createWindows = () => {
  window$1 = new require$$3.BrowserWindow({
    width: 1400,
    height: 800,
    minWidth: 900,
    minHeight: 660,
  ...
  if (process.env.NODE_ENV !== "development") {
    window$1.loadFile(path.join(path.dirname(__dirname), "/dist/index.html"));
  } else {
    window$1.loadURL("http://localhost:4396");
    window$1.webContents.openDevTools();
  }
  remoteMain.enable(window$1.webContents);
};
...
require$$3.app.on("activate", () => {
  if (require$$3.BrowserWindow.getAllWindows.length === 0) createWindows();
});
